...
public class RightArrawGuide : GuideLine
{
    ...
    protected override void SetShapePoints()
    {
        ...
        triangleVertex.Add(Vector2.down * shapeSize);

        float diagonalLenght = Mathf.Sqrt(Mathf.Pow(shapeSize, 2) / 2f);
        //사각형
        for (int i = 0; i < 4; i++)
        {
            squareVertex.Add(((Vector2)(Quaternion.Euler(0, 0, 45 + 90 * i) * Vector2.up) * diagonalLenght) + Vector2.right * -(shapeSize / 2));
        }

        DrawLine(triangleVertex[0], triangleVertex[1]);
        DrawLine(triangleVertex[1], squareVertex[3]);
        DrawLine(squareVertex[3], squareVertex[0]);
        DrawLine(squareVertex[0], squareVertex[1]);
        DrawLine(squareVertex[1], squareVertex[2]);
        DrawLine(squareVertex[2], triangleVertex[2]);
